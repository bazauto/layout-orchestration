...
| D8 | The panel keeps its own injection history client-side, labelled "last injected", never "current" | **Restated by #76**, which put per-sensor readings on `STATE_SNAPSHOT` and superseded the `DD10` exclusion this row used to cite. That citation no longer supports the conclusion, but the conclusion stands on its own: a record of *your own keystrokes* is not a claim about the layout, and correctly resets on reload — where "current", the word #76's `SensorObservationView` now legitimately owns, would claim to be live and would need to be kept honest across a reconnect that #76 gives it no way to hear about |
...
- **A `STATE_SNAPSHOT`/`SENSOR_STATE` change to THIS feature** — #76 added
  per-sensor readings to the snapshot for the monitor's sensor layer, superseding
  the `DD10` exclusion this bullet used to cite (see D8's rewritten reasoning
  above), but that is #76's surface, not this one's: `SensorSimulationService`
  still only publishes to MQTT and reads nothing back off the wire it just wrote.
