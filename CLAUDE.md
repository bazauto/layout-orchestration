...
- **A point lock is an authority guarantee, and now a position guarantee too — but only for
  ...
  the live layout is `'none'` until an operator changes it. **What PR A does not yet do is name
  which route a failed point invalidated**: a `PointFault` Safe-Stops the system, which suspends
  every active route with locks retained, but the route-scoped fault, the resume precondition
  and `docs/route-locking.md` D11's rewrite are PR B (`docs/point-feedback.md` D8).
