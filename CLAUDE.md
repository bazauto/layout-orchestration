...
- Phase 3's remaining work is **collision avoidance and speed control (#7)**, which needs
  ...
  complete: what it deliberately does not do is decide *when* to brake. #77 is **done bar
  the authoring form**: PR A landed the model, PR B the braking consumption that unblocks
  #7, PR C is the Configure → Sensors fields (`docs/plans/77-sensor-position.md`). The
  scenario harness (#5) shipped long ago and its issue is closed.
