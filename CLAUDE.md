...
| `docs/track-graph-compilation.md` | **Accepted design, mostly built (#103).** The drawing *compiles* to `block_edges` under operator review; `block_ends` is deleted (D1–D10). **Shipped: D4/D5, the compiler, both read surfaces, the apply, the `auto` gate, and the review UI — the compiler is now the only writer of `block_edges`.** Not yet: deleting `block_ends`. Status table at the head of the file. Read before touching #72. |
...
| `docs/track-editor.md` | Track Editor authoring: derived canvas extent, per-stroke undo, view persistence, the wave-2 affordances, diagram labelling, keyboard navigation, and the hand-naming of block ends (D1–D13) |
...
| **Track Editor authoring** (#69, #94, #103 step 6.1) | Canvas extent derives from content plus a margin — there is no fixed grid to hit, and `MAX_COORDINATE` is admission control, not an edge. Undo is client-side and **per stroke**, so a stray right-drag is one click to reverse; it is never persisted. `⌂` fits content. Pan/zoom persist per layout. The canvas is keyboard-navigable (`role="application"`): a `cursor` state moves on arrow keys, paints on Enter/Space, erases on Delete, and Escape exits to the toolbar; one `aria-live` string (`diagram/cursorAnnouncement.ts`) is both the visible readout and the screen-reader announcement. Ruler gutters and a crosshair give a persistent spatial reference; diagnostics with a coordinate (`diagnosticCoordinate`) are click-to-jump buttons, and `jumpToCell` is the one implementation both they and the ends panel use. Each compiled opening (`GET .../grid/openings`, D-H) draws a boundary tick at every port it names (`diagram/openings.ts#portMarkGeometry`, pure, no rotation of its own — `Port.edge` is already screen-frame) plus the existing `⊣` stop glyph on a terminated one and its label once at `opening.at`, replacing the old label-at-a-nearby-cell render (#91's fused-siding argument, `docs/diagram-encoding.md` D7). `Ends ✎` still opens `BlockEndsPanel` — create, rename/pin and delete a `block_ends` row, with the 409 that refuses a rename of an edge-referenced label rendered verbatim — unchanged for now; #103 step 6.2 removes it. A list of controls, never a click on the drawn label: the canvas is `role="application"` and a click there paints. | `docs/track-editor.md`, `docs/diagram-encoding.md` D7 |
...
- **The drawing↔graph model has been replaced (#103, `docs/track-graph-compilation.md`),
  except for deleting `block_ends`.** Accepted 2026-08-13. Shipped: D4/D5 (#105 — length
  is on `blocks.length_mm`, joints are zero), the #104 fix in the walk, the compiler, both
  read surfaces, the apply, the `auto` gate and the review UI. **The compiler is the only
  writer of `block_edges`** — the manual write path and the edge-proposal surface are both
  deleted (PR 5). Still standing, and deleted by PR 6.2/6.3 and PR 7: `block_ends`,
  `BlockEndService`, its routes, and the `Ends ⟳`/`Ends ✎` controls. The root fault they
  carry: `block_ends.label` is both the join key `block_edges` references and a
  geometry-derived description, and `pinned`, the rename 409, adoption and the collision
  refusal are all that conflict surfacing. **Do not build further on #72's model** —
  including the anchor coordinate in #97, which is rejected. The plan for the remaining
  PRs is `docs/plans/103-track-graph-compilation.md`, and each shipped PR carries a
  "Shipped, with notes" block recording what differed from it.
...
- **An end the generator refused to name cannot be placed on the drawing.** An
  ...
  end matching one of them lists as "not placed" and reports
  `pinned-end-not-on-diagram`. It no longer affects the graph — `compileOpenings`
  disambiguates by suffix and names every opening — so this is now a `block_ends`
  artefact only, and dies with it in PR 7. Westgate Hollow has exactly one (`Engine / Goods Transfer`, two openings both
