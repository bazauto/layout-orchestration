...
- Phase 3's remaining work is **collision avoidance and speed control (#7)**, in progress
  against `docs/plans/7-automation.md`. **PR A (the model) and PR B (the engine) have
  landed**; PR C is the operator surface. Automation now drives an `auto`-authority route
  end to end — but **Westgate Hollow's behaviour is unchanged until an operator opts in**:
  ...
  no `auto` route. Its dependencies are all closed: per-loco braking (#6) is complete apart from deliberately
  not deciding *when* to brake, and **#77 is done** — PR A the model, PR B the braking
  consumption, PR C the authoring column (`docs/plans/77-sensor-position.md`), leaving only
  the fitting and measuring of beams. The scenario harness (#5) shipped long ago and its
  issue is closed.
