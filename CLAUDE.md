...
| **Sensor trust** (#28) | A retained reading is not evidence. Hardware must re-assert every 30 s; `domain/sensorTrust.ts#isSensorFresh` plus a trust sweep on `IClock` set `trusted`. Per-sensor observations are on the WebSocket snapshot and a `SENSOR_STATE` delta (#76), pushed only on a contributed-value change. | `sensor-trust.md`, `sensor-fault-recovery.md` D10 |
...
| **Diagram encoding** (#81, #68, #93, #76) | `diagram/encoding.ts` is the single source of diagram colour; every state ships a pattern/glyph/label. Four validator-checked block tints — do not add a fifth. **Texture marks a fault state, never an operational one**: `unknown` is the only hatch left, `occupied` and `clear` are flat washes separated by `OCCUPANCY_WASH_OPACITY` (D10). Live sensor observation is its own channel on the placed-sensor glyph, never a block tint, and on the control view it is **always drawn** — the checkbox that gated it is withdrawn (D9). | `diagram-encoding.md` |
