...
- Phase 3's remaining work is **collision avoidance and speed control (#7)**, now in
  progress against `docs/plans/7-automation.md`. **PR A (the model) has landed and is
  inert** — nothing sweeps, nothing departs, and no train moves that would not have moved
  before. PR B wires the sweep and the execution; PR C is the operator surface. Its
  dependencies are all closed: per-loco braking (#6) is complete apart from deliberately
