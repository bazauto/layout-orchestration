...
| **Diagram encoding** (#81, #68, #93, #76) | `diagram/encoding.ts` is the single source of diagram colour; every state ships a pattern/glyph/label. Four validator-checked block tints — do not add a fifth. Live sensor observation is its own channel on the placed-sensor glyph, never a block tint. | `diagram-encoding.md` |
