...
- **Two hand-maintained duplicates across the wire**, and **#75 did not close them**:
  ...
  **Two of these changed shape** (`docs/track-editor.md` D16). `EDGE_OFFSET`'s mirror is
  **closed** — it existed only for `portMarkGeometry`, which is deleted with the opening marks
  (D15). And the `TILE_LEGS` mirror was `DRAWN_LEGS` in `diagram/pointRoads.ts`, *two rows* of
