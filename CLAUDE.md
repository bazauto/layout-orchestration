...
| `docs/track-graph-compilation.md` | **Accepted design, part built (#103).** The drawing *compiles* to `block_edges` under operator review; `block_ends` is deleted (D1–D10). **Shipped: D4/D5** (length on blocks, joints zero), **the compiler, both read surfaces, the apply, and the `auto` gate.** The review UI and the deletions have not. Status table at the head of the file. Read before extending #72 or #78. |
...
- **The drawing↔graph model is being replaced (#103, `docs/track-graph-compilation.md`).**
  ...
  reuses, and so have the compiler, both read surfaces and **the apply** (row above).
  Still to come: the review UI, and the deletions —
  `block_ends`, `BlockEndService`, the edge-proposal surface and the manual edge write
  path, which all still work. A compiled apply and a hand-authored edge can both write
  `block_edges` today; that is temporary, and the compiler wins by deleting the other
  one (PR 5), not by guarding against it. The root fault:
...
- **Westgate Hollow's classification pass is done** — 0 unclassified tiles, every block
  ...
  pressed apply on the live layout yet. The apply endpoint now exists, so that is a
  decision waiting on an operator, not a missing capability.
