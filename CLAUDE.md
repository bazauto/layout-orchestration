...
- **The drawing↔graph model has been replaced (#103, `docs/track-graph-compilation.md`),
  ...
  read surfaces, the apply, the `auto` gate, the review UI, and the Track Editor's side of
  it. **The compiler is the only writer of `block_edges`** — the manual write path and the
  edge-proposal surface are both deleted (PR 5), and the `Ends ⟳`/`Ends ✎` controls with
  them (PR 6.2). Still standing, called by nothing in the browser, and deleted by PR 7:
  `block_ends`, `BlockEndService` and its routes. The root fault they carry:
  ...
  that conflict surfacing. **Do not build further on #72's model** — including the anchor
  coordinate in #97, which is rejected. The plan for the remaining PRs is
  `docs/plans/103-track-graph-compilation.md`, and each shipped PR carries a
