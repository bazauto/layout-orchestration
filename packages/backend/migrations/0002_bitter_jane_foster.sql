...
CREATE TABLE `__new_block_edges` (
	...
	CONSTRAINT "block_edges_length_positive" CHECK("__new_block_edges"."length_mm" IS NULL OR "__new_block_edges"."length_mm" > 0),
...
INSERT INTO `__new_block_edges`("id", "layout_id", "from_block_id", "from_end", "to_block_id", "to_end", "point_conditions", "length_mm") SELECT "id", "layout_id", "from_block_id", "from_end", "to_block_id", "to_end", "point_conditions", "length_mm" FROM `block_edges`;--> statement-breakpoint
...
CREATE UNIQUE INDEX `block_edges_connection_unq` ON `block_edges` (`layout_id`,`from_block_id`,`from_end`,`to_block_id`,`to_end`);