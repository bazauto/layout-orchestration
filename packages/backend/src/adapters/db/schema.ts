...
export const sensors = sqliteTable('sensors', {
  ...
  /**
   * Sub-block position (#77, docs/sensor-position.md D1): this sensor lies
   * `position_offset_mm` of track before the boundary its own block shares
   * with `position_toward_block_id`.
   *
   * Anchored to a **neighbouring block id**, not to a block-end label, because
   * a label is regenerated by every compile and a stored one would silently
   * come to name a different end (D2). A block id is operator-owned and the
   * compiler never rewrites it.
   *
   * Both NULL = unmeasured, which reproduces `docs/braking.md` B4's worst case
   * exactly (D3) — the same posture as `blocks.length_mm`. Nothing back-fills
   * them, so the live layout behaves identically the instant after this
   * migration runs.
   *
   * `ON DELETE SET NULL` mirrors `block_id` above: deleting the anchor block
   * leaves a half-set pair, which reads as **unmeasured**, not as corruption
   * (D5). That is deliberately unlike every other column pair here — a
   * fail-safe reading exists, and refusing to list sensors over a measurement
   * would take the layout down for a concession it can simply decline to make.
   *
   * Deliberately no CHECK constraint, following DD9's call on `in_service`: a
   * CHECK on an existing SQLite table forces a table rebuild on a database
   * deployed to a live layout. `sensorCreateSchema`/`sensorUpdateSchema` and
   * `LayoutService`'s cross-row rules (D4/D5) already refuse a bad value on
   * every write path.
   */
  positionTowardBlockId: text('position_toward_block_id').references(() => blocks.id, {
    onDelete: 'set null',
  }),
  positionOffsetMm: integer('position_offset_mm'),
});
