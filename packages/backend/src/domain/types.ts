...
/**
 * Where a sensor is within its block (#77, `docs/sensor-position.md` D1):
 * `offsetMm` of track before the boundary its own block shares with
 * `towardBlockId`.
 *
 * Anchored to a **neighbouring block id**, never to a block-end label. A label
 * is disposable compiler output regenerated by every compile (D8,
 * `docs/track-graph-compilation.md`), so a stored one comes to name a different
 * end after an innocuous redraw — an offset measured from the wrong end of a
 * block is exactly the error that stops a train in the wrong place. A block id
 * is operator-owned and the compiler never rewrites it (D2).
 *
 * The pair is meaningful only against the live graph: it resolves to nothing
 * when the two blocks are not connected, or are connected in more than one
 * place (D5). See `domain/sensorPosition.ts`.
 */
...
export interface SensorObservation {
  ...
  /**
   * When this sensor last went `clear → occupied` — the ONLY moment a position
   * fix is taken (#77 D6). Deliberately distinct from `lastReadingAt`, which
   * moves on every reading: a device re-publishing `occupied` on a timer would
   * otherwise appear to re-observe a train that has long since gone.
   *
   * Runtime state, never persisted (D11), and cleared with the reading when the
   * sensor faults or leaves service.
   */
  lastRisingEdgeAt: Date | null;
}

/**
 * "The leading edge of a train was at this spot at this instant" (#77 D6) —
 * the only positional statement this system makes, and an observation rather
 * than a state. It is worth less as it ages, by the bound D7 describes; it is
 * never accumulated into a running estimate of where a train is now.
 */
