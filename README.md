...
- Sub-block sensor position (#77, PR A): an IR beam can carry a measurement — how far it
  sits from the boundary with a *named neighbouring block* — and a `clear → occupied`
  transition is treated as a position fix at that instant, decaying by a worst-case travel
  allowance rather than expiring on a timer. Anchored to a block id, never to a compiled
  block-end label, which is regenerated by every compile. Unmeasured is the default and
  behaves exactly as before. **Nothing consumes a fix yet** — see `docs/sensor-position.md`
...
prerequisite for #7 rather than a refinement of it. See `docs/braking.md` B4. #77's model
is now in place — an IR beam can say how far it is from the boundary with a *neighbouring
block* — but nothing consumes it yet, so the refusal above still stands as written.
...
1. Sub-block position (#77) — the model has landed (`docs/sensor-position.md`); what is
   left is the braking consumption and an authoring form. Then collision avoidance and
   speed control (#7) on top of it. Per-loco braking (#6) is done
